...
class TestCachedPlan:
    ...
    def test_from_dict_ignores_extra_keys(self):
        ...
        assert not hasattr(plan, "extra_field"), "CachedPlan.from_dict must filter unknown keys"
...
class TestPlanCache:
    ...
    def test_find_similar_above_threshold(self):
        ...
        assert result is not None
        assert result.goal == "build user authentication system"
        assert isinstance(result.plan_data, dict)
    ...
    def test_persistence_across_instances(self):
        ...
        assert result is not None
        assert result.goal == "persistent goal"
        assert result.plan_data == {"tasks": [1]}
