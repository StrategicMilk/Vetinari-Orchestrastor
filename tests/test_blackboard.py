...
class TestBlackboardEntry:
    def test_required_fields_stored(self):
        e = _make_entry(entry_id="id1", content="hello", request_type="rt", requested_by="A")
    ...
    def test_is_expired_uses_time_time(self):
        ...
        with patch("vetinari.blackboard.time") as mock_time:
    ...
    def test_to_dict_keys(self):
        ...
        d = e.to_dict()
        expected_keys = {
            "entry_id", "content", "request_type", "requested_by",
            "priority", "state", "claimed_by", "result", "error",
            "created_at", "completed_at", "ttl_seconds",
        }
    ...
    def test_to_dict_entry_id_matches(self):
        e = _make_entry(entry_id="specific_id")
...
class TestBlackboardPost:
    def test_returns_string_entry_id(self, board):
        eid = board.post("content", "code_search", "BUILDER")
    ...
    def test_entry_custom_priority(self, board):
        eid = board.post("content", "code_search", "BUILDER", priority=1)
    ...
    def test_entry_custom_ttl(self, board):
        eid = board.post("content", "code_search", "BUILDER", ttl_seconds=60.0)
        assert board.get_entry(eid).ttl_seconds == 60.0

    def test_entry_metadata_stored(self, board):
        eid = board.post("content", "code_search", "BUILDER", metadata={"key": "val"})
    ...
    def test_unique_ids_per_post(self, board):
        ids = {board.post(f"content {i}", "code_search", "BUILDER") for i in range(20)}
        assert len(ids) == 20
...
class TestBlackboardGetPending:
    ...
    def test_filters_by_request_type(self, board):
        eid1 = board.post("a", "code_search", "BUILDER")
        eid2 = board.post("b", "code_review", "BUILDER")
    ...
    def test_sorted_by_priority_ascending(self, board):
        board.post("low", "code_search", "BUILDER", priority=9)
        board.post("high", "code_search", "BUILDER", priority=1)
        board.post("mid", "code_search", "BUILDER", priority=5)
        pending = board.get_pending(request_type="code_search")
    ...
    def test_respects_limit(self, board):
        for i in range(10):
            board.post(f"task {i}", "code_search", "BUILDER")
        pending = board.get_pending(limit=3)
        assert len(pending) <= 3

    def test_excludes_expired_entries(self, board):
        eid = board.post("expired task", "code_search", "BUILDER", ttl_seconds=1.0)
        entry = board.get_entry(eid)
...
class TestBlackboardDelegate:
    ...
    def test_delegate_uses_planner_first(self, board):
        ...
        with patch("vetinari.agents.contracts.AgentTask.from_task",
                   return_value=mock_agent_task):
            task = self._make_task()
            available_agents = {self.AgentType.PLANNER: mock_agent}
    ...
    def test_delegate_falls_back_to_builder(self, board):
        ...
        with patch("vetinari.agents.contracts.AgentTask.from_task",
                   return_value=mock_agent_task):
            task = self._make_task()
            # Planner not available; builder is
            available_agents = {self.AgentType.BUILDER: mock_agent}
            result = board.delegate(task, available_agents)
    ...
    def test_delegate_falls_back_to_researcher(self, board):
        ...
        with patch("vetinari.agents.contracts.AgentTask.from_task",
                   return_value=mock_agent_task):
            task = self._make_task()
            # Only researcher available
            available_agents = {self.AgentType.RESEARCHER: mock_agent}
            result = board.delegate(task, available_agents)
    ...
    def test_delegate_continues_on_agent_exception(self, board):
        ...
        with patch("vetinari.agents.contracts.AgentTask.from_task",
            ...
            task = self._make_task()
            available_agents = {
                self.AgentType.PLANNER: failing_agent,
                self.AgentType.BUILDER: working_agent,
            }
            result = board.delegate(task, available_agents)
...
class TestBlackboardRequestHelp:
    def test_request_help_posts_and_waits(self, board):
        ...
        def _complete_after_post():
            # Wait until an entry has been posted
            while not posted_ids:
                time.sleep(0.01)
...
class TestBlackboardEscalateError:
    def test_escalate_returns_entry_id(self, board):
        eid = board.escalate_error("BUILDER", "task_1", "NullPointerError")
        assert isinstance(eid, str)
        assert eid.startswith("bb_")
    ...
    def test_escalate_sets_high_priority(self, board):
        eid = board.escalate_error("BUILDER", "task_1", "err")
        assert board.get_entry(eid).priority == 1

    def test_escalate_metadata_contains_original_task_id(self, board):
        eid = board.escalate_error("BUILDER", "task_42", "err")
        entry = board.get_entry(eid)
        assert entry.metadata["original_task_id"] == "task_42"

    def test_escalate_metadata_contains_error(self, board):
        eid = board.escalate_error("BUILDER", "task_1", "something went wrong")
        entry = board.get_entry(eid)
        assert entry.metadata["error"] == "something went wrong"

    def test_escalate_metadata_includes_context(self, board):
        eid = board.escalate_error("BUILDER", "task_1", "err", context={"trace": "abc"})
        entry = board.get_entry(eid)
...
class TestBlackboardRequestConsensus:
    ...
    def test_request_consensus_extra_metadata_merged(self, board):
        eid = board.request_consensus(
            "ORACLE", "s", ["A"], metadata={"urgency": "high"}
        )
        entry = board.get_entry(eid)
...
class TestObserverPattern:
    def test_subscribe_registers_callback(self, board):
        cb = MagicMock()
        board.subscribe(cb)
        board.post("content", "code_search", "BUILDER")
    ...
    def test_no_observers_is_safe(self, board):
        # Post with no observers should not raise
        board.post("content", "code_search", "BUILDER")
...
class TestBlackboardPurgeExpired:
    def test_purge_marks_expired_entries(self, board):
        eid = board.post("stale", "code_search", "BUILDER", ttl_seconds=1.0)
        entry = board.get_entry(eid)
    ...
    def test_purge_does_not_affect_fresh_entries(self, board):
        eid = board.post("fresh", "code_search", "BUILDER", ttl_seconds=3600.0)
        board.purge_expired()
        assert board.get_entry(eid).state == EntryState.PENDING

    def test_purge_returns_count_of_expired(self, board):
        for i in range(3):
            eid = board.post(f"stale {i}", "code_search", "BUILDER", ttl_seconds=1.0)
            board.get_entry(eid).created_at = time.time() - 100.0
        count = board.purge_expired()
        assert count == 3

    def test_purge_removes_stale_completed_older_than_2h(self, board):
        eid = board.post("old completed", "code_search", "BUILDER")
        entry = board.get_entry(eid)
    ...
    def test_purge_removes_stale_failed_older_than_2h(self, board):
        eid = board.post("old failed", "code_search", "BUILDER")
        entry = board.get_entry(eid)
    ...
    def test_purge_does_not_remove_recent_completed(self, board):
        eid = board.post("recent completed", "code_search", "BUILDER")
        board.complete(eid, result="ok")
        board.purge_expired()
        assert board.get_entry(eid) is not None

    def test_purge_returns_zero_when_nothing_expired(self, board):
        board.post("fresh", "code_search", "BUILDER")
...
class TestSharedExecutionContext:
    ...
    def test_get_all_by_agent_filters(self, ctx):
        ctx.set("x", 10, "BUILDER")
        ctx.set("y", 20, "RESEARCHER")
        ctx.set("z", 30, "BUILDER")
        result = ctx.get_all_by_agent("BUILDER")
    ...
    def test_clear_removes_provenance(self, ctx):
        ctx.set("a", 1, "BUILDER")
        ctx.clear()
        assert ctx.get_all_by_agent("BUILDER") == {}
...
class TestRequestTypeRouting:
    ...
    def test_known_type_code_search(self):
        agents = get_capable_agents("code_search")
        assert len(agents) > 0
        assert "EXPLORER" in agents or "CONSOLIDATED_RESEARCHER" in agents

    def test_known_type_code_review(self):
        agents = get_capable_agents("code_review")
        assert len(agents) > 0

    def test_known_type_error_recovery(self):
        agents = get_capable_agents("error_recovery")
        assert "ERROR_RECOVERY" in agents or "OPERATIONS" in agents

    def test_known_type_implementation(self):
        agents = get_capable_agents("implementation")
...
class TestThreadSafety:
    ...
    def test_concurrent_claim_only_one_wins(self, board):
        ...
        threads = [
            threading.Thread(target=_claimer, args=(f"AGENT_{i}",), daemon=True)
            for i in range(20)
        ]
        for t in threads:
            t.start()
    ...
    def test_concurrent_post_and_get_stats(self, board):
        ...
        threads = (
            [threading.Thread(target=_poster, daemon=True) for _ in range(5)] +
