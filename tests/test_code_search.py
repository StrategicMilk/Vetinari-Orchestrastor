...
class TestCocoIndexSupplyChain:
    def test_unconfigured_cocoindex_uses_regex_fallback(self, tmp_path):
        target = tmp_path / "sample.py"
        target.write_text("def needle():\n    return 'ok'\n", encoding="utf-8")

        with patch.dict("os.environ", {}, clear=True):
            adapter = CocoIndexAdapter(root_path=str(tmp_path))
            results = adapter.search("needle")

        assert adapter._last_backend == "regex_fallback"
        assert [result.file_path for result in results] == [str(target)]

    def test_latest_package_spec_is_rejected(self, tmp_path):
        with patch.dict("os.environ", {"VETINARI_COCOINDEX_CODE_PACKAGE": "cocoindex-code@latest"}):
            adapter = CocoIndexAdapter(root_path=str(tmp_path))
            assert adapter.get_status() == SearchBackendStatus.UNAVAILABLE
