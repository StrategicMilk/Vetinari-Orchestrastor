...
class TestOperationsSkillToolMetadata:
    ...
    def test_output_format_parameter(self):
        ...
        fmt_param = next(p for p in tool.metadata.parameters if p.name == "output_format")
...
class TestOperationsSkillToolExecution:
    ...
    def test_image_generation(self):
        """Test image generation mode — engine unavailable path returns success=False."""
        from unittest.mock import patch

        from vetinari.image.diffusion_engine import DiffusionEngine

        with patch.object(DiffusionEngine, "is_available", return_value=False):
            result = self.tool.execute(
...
class TestOperationsSkillToolEdgeCases:
    ...
    def test_all_modes_execute_successfully(self):
        ...
        # Provide a mock engine so image_generation returns success=True
        mock_engine = MagicMock(spec=DiffusionEngine)
        ...
        with patch("vetinari.image.diffusion_engine.DiffusionEngine", return_value=mock_engine):
            for mode in OperationsMode:
                result = self.tool.execute(
                    mode=mode.value,
