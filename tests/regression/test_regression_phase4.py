...
class TestAlertEngineContract:
    ...
    @pytest.fixture
    def mock_api(self):
        """Return a factory that builds a mock DashboardAPI stub with a given latency."""

        def _factory(latency: float = 100.0) -> MagicMock:
            snap = MagicMock()
    ...
    def test_evaluate_fires_and_suppresses(self, mock_api):
        self.engine.register_threshold(
            AlertThreshold(
                name="t",
                metric_key="adapters.average_latency_ms",
        ...
        fired1 = self.engine.evaluate_all(api=mock_api(latency=200.0))
        fired2 = self.engine.evaluate_all(api=mock_api(latency=200.0))
        assert len(fired1) == 1
        assert len(fired2) == 0

    def test_evaluate_clears_after_condition_resolves(self, mock_api):
        self.engine.register_threshold(
            AlertThreshold(
                name="t",
                metric_key="adapters.average_latency_ms",
        ...
        self.engine.evaluate_all(api=mock_api(latency=200.0))
        self.engine.evaluate_all(api=mock_api(latency=10.0))
        assert len(self.engine.get_active_alerts()) == 0

    def test_singleton_contract(self):
        assert get_alert_engine() is get_alert_engine()

    def test_alert_record_to_dict(self, mock_api):
        self.engine.register_threshold(
            AlertThreshold(
                name="t2",
                metric_key="adapters.average_latency_ms",
        ...
        fired = self.engine.evaluate_all(api=mock_api(latency=200.0))
        assert len(fired) == 1
        d = fired[0].to_dict()
...
class TestLogAggregatorContract:
    @pytest.fixture(autouse=True)
    def _setup(self):
        reset_log_aggregator()
    ...
    def test_search_returns_newest_first(self):
        for i in range(5):
            r = LogRecord(message=f"msg-{i}", trace_id="r-002", timestamp=float(i))
            self.agg.ingest(r)
