...
class TestExecutionContextIntegration(unittest.TestCase):
    ...
    def test_context_stacking(self):
        ...
        initial_depth = len(self.context_manager._context_stack)

        # Switch to planning
        self.context_manager.switch_mode(ExecutionMode.PLANNING)
        assert len(self.context_manager._context_stack) == initial_depth + 1

        # Switch to execution (nested)
        self.context_manager.switch_mode(ExecutionMode.EXECUTION)
        assert len(self.context_manager._context_stack) == initial_depth + 2
        ...
        self.context_manager.pop_context()
        assert len(self.context_manager._context_stack) == initial_depth
...
class TestAdapterManagerWithExecutionContext(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        while len(self.context_manager._context_stack) > 1:
            self.context_manager.pop_context()
    ...
    def test_infer_respects_execution_mode(self):
        ...
        # This depends on the default policy for MODEL_INFERENCE in PLANNING mode
...
class TestToolInterfaceIntegration(unittest.TestCase):
...
class TestVerificationPipelineIntegration(unittest.TestCase):
...
class TestPermissionEnforcementAcrossComponents(unittest.TestCase):
    ...
    def test_permission_denied_in_sandbox_mode(self):
        ...
        # The exact permissions depend on the SANDBOX policy
