...
class TestHeartbeat:
    ...
    def test_heartbeat_registered_succeeds(self):
        ...
        # Verify agent is still in healthy state after heartbeat
        unhealthy = self.monitor.check_health()
        assert all(u["agent_id"] != "worker-1" for u in unhealthy)
...
class TestStepLimit:
    ...
    def test_steps_within_limit_do_not_raise(self):
        ...
        stats = self.monitor.get_stats()
        assert stats["total_steps_recorded"] >= 10
...
class TestResetAgent:
    ...
    def test_reset_clears_step_count(self):
        ...
        # After reset, agent has 1 step recorded (below max_steps=2)
        stats = self.monitor.get_stats()
        assert stats["total_steps_recorded"] >= 1
...
class TestMonitorStats:
    ...
    def test_step_limit_violation_counted(self):
        ...
        try:
            self.monitor.record_step("v-agent")
        except StepLimitExceeded:  # noqa: VET022 - best-effort optional path must not fail the primary flow
            pass
        stats = self.monitor.get_stats()
...
class TestPolicyEnforcerSingleton:
    @pytest.fixture(autouse=True)
    def _setup(self):
        reset_policy_enforcer()
        yield
...
class TestFileJurisdiction:
    ...
    def test_worker_denied_outside_jurisdiction(self):
        decision = self.enforcer.check_action(
            agent_type="worker",
            action="write",
            target=".omc/plans/plan.json",
            context={},
        )
        assert decision.allowed is False
        assert "worker" in decision.reason.lower()

    def test_inspector_allowed_in_reviews(self):
        decision = self.enforcer.check_action(
            agent_type="inspector",
            action="write",
            target=".omc/reviews/quality_report.md",
            context={},
        )
        assert decision.allowed is True

    def test_inspector_denied_in_vetinari_source(self):
        decision = self.enforcer.check_action(
            agent_type="inspector",
            action="write",
...
class TestEnforcerStats:
    ...
    def test_total_checks_increments(self):
        ...
        self.enforcer.check_action("worker", "read", "any/file", {})
        self.enforcer.check_action("worker", "read", "any/file", {})
...
class TestSafetyModuleExports:
    def test_imports_from_safety_package(self):
        ...
        assert callable(AgentMonitor)
        assert issubclass(StepLimitExceeded, Exception)
        assert callable(get_agent_monitor)
