...
from tests.factories import make_stub_module

# ---------------------------------------------------------------------------
# Stub heavy ML libs so the module can be imported without them
# ---------------------------------------------------------------------------

for _lib in ["unsloth", "trl", "peft", "transformers", "bitsandbytes", "datasets", "torch"]:
    if _lib not in sys.modules:
        sys.modules[_lib] = make_stub_module(_lib)
...
_fake_training_data_mod = make_stub_module("vetinari.learning.training_data")
...
_learning_stub = make_stub_module("vetinari.learning")
...
import vetinari as _vetinari_pkg
...
# ---------------------------------------------------------------------------
# Package installer guard
# ---------------------------------------------------------------------------


def test_ensure_packages_refuses_unapproved_package() -> None:
    """Package auto-install must be constrained to known optional extras."""
    with patch("subprocess.run") as mock_run:
        result = _ensure_packages(["not-a-training-extra"])

    assert result == {"not-a-training-extra": False}
...
class TestTrainingRun:
    ...
    def test_set_eval_score(self):
        ...
        assert run.eval_score == pytest.approx(0.87)
...
class TestDataCuratorCurate:
    ...
    def test_multiple_records_written(self):
        records = [{"prompt": f"prompt {i}", "completion": f"output {i}"} for i in range(10)]
        path = self._curate(records)
        with open(path) as f:
            lines = f.readlines()
        assert len(lines) == 10
    ...
    def test_prompt_truncated_at_output_summary_limit(self):
        long_prompt = "x" * 8000
        records = [{"prompt": long_prompt, "completion": "out"}]
        path = self._curate(records)
        with open(path) as f:
            obj = json.loads(f.readline())
        assert len(obj["instruction"]) == 4000

    def test_completion_truncated_at_output_preview_limit(self):
        long_comp = "y" * 12000
...
class TestDataCuratorCurateDpo:
    ...
    def test_each_line_valid_json(self):
        ...
        with open(path, encoding="utf-8") as f:
            for line in f:
                ...
                assert "prompt" in data, f"Missing 'prompt' key in JSONL line: {data}"
                line_count += 1
        assert line_count == len(pairs), f"Expected {len(pairs)} lines, got {line_count}"
...
class TestLocalTrainerCheckAvailable:
    ...
    def _check_with_libs(self, available_libs):
        """Patch module discovery so only libs in available_libs appear installed."""
    ...
    def test_missing_lib_returns_false(self):
        """Force ImportError for a specific lib — result must reflect unavailability."""
        original = sys.modules.pop("unsloth", None)
        try:
            sys.modules["unsloth"] = None  # type: ignore[assignment]
            result = self.trainer.check_available()
            assert isinstance(result, dict), f"check_available must return dict, got {type(result)}"
        finally:
...
class TestLocalTrainerTrainQlora:
    ...
    def test_subprocess_run_called_once_unsloth(self):
        ...
        assert result.endswith("lora_adapter")
        assert cmd == [sys.executable, str(Path(self.tmpdir) / "train_script.py")]
        assert kwargs["capture_output"] is True
        assert kwargs["text"] is True

    def test_subprocess_run_called_once_trl(self):
        avail = self._avail(unsloth=False, trl=True)
        result, mock_sub = self._run_qlora(avail, use_unsloth=False)
        cmd = mock_sub.call_args[0][0]
        kwargs = mock_sub.call_args.kwargs
        assert result.endswith("lora_adapter")
        assert cmd == [sys.executable, str(Path(self.tmpdir) / "train_trl_script.py")]
        assert kwargs["capture_output"] is True
        assert kwargs["text"] is True
    ...
    def test_unsloth_script_uses_requested_revision(self):
        revision = "a" * 40
        avail = self._avail(unsloth=True, trl=True)
        self._run_qlora(avail, use_unsloth=True, model_revision=revision)
        script_text = (Path(self.tmpdir) / "train_script.py").read_text()
        assert f'revision="{revision}"' in script_text
        assert 'revision="main"' not in script_text

    def test_trl_script_uses_requested_revision(self):
        revision = "b" * 40
        avail = self._avail(unsloth=False, trl=True)
        self._run_qlora(avail, use_unsloth=False, model_revision=revision)
        script_text = (Path(self.tmpdir) / "train_trl_script.py").read_text()
        assert f'revision="{revision}"' in script_text
        assert 'revision="main"' not in script_text
...
class TestGGUFConverter:
    ...
    def test_merge_script_contains_adapter_path(self):
        self._convert()
        text = (Path(self.tmpdir) / "merge_script.py").read_text(encoding="utf-8")
        # Paths are JSON-encoded in the generated script to handle backslashes
        import json

        assert json.dumps(self.adapter_path) in text or self.adapter_path.replace("\\", "\\\\") in text
    ...
    def test_merge_script_uses_requested_revision_for_base_tokenizer(self):
        revision = "c" * 40
        proc1 = MagicMock(returncode=0, stderr="")
        proc2 = MagicMock(returncode=0, stderr="")
        with patch("subprocess.run", side_effect=[proc1, proc2]):
...
class TestModelDeployer:
    ...
    def test_shutil_copy2_called(self):
        result, mock_copy = self._deploy()
        copied_src, copied_dest = mock_copy.call_args[0]
        assert copied_src == self.gguf_path
        assert copied_dest == result
        assert result.endswith("model_q4_k_m.gguf")
    ...
    def test_deploy_native_writes_training_manifest(self):
        adapter_dir = Path(self.tmpdir) / "adapter"
        adapter_dir.mkdir()
        (adapter_dir / "adapter_model.safetensors").write_bytes(b"adapter")
        native_root = Path(self.tmpdir) / "native"

        with patch("vetinari.training.pipeline_trainers._NATIVE_MODELS_DIR", native_root):
            result = self.deployer.deploy_native(
                str(adapter_dir),
                "native model",
                backend="vllm",
        ...
        assert manifest["base_model_revision"] == "d" * 40
...
class TestTrainingPipelineRun:
    ...
    def _run_pipeline(self, n_records=50, train_rc=0, merge_rc=0, convert_rc=0, model_name_suffix="7B", **kw):
        ...
        kw.setdefault("model_revision", "e" * 40)
        ...
        def fake_deploy_native(adapter_path, model_name, **kwargs):
            return {
                "path": os.path.join(self.tmpdir, "native_adapter"),
                "manifest_path": os.path.join(self.tmpdir, "native_adapter", ".vetinari-training-manifest.json"),
            }

        with patch("vetinari.learning.training_data.get_training_collector", return_value=collector):
            with patch("subprocess.run", side_effect=fake_subprocess_run):
                with patch.object(self.pipeline._deployer, "deploy", side_effect=fake_deploy):
                    with patch.object(self.pipeline._deployer, "deploy_native", side_effect=fake_deploy_native):
    ...
    def test_default_backend_is_vllm_safetensors(self):
        run = self._run_pipeline()
        assert run.backend == "vllm"
        assert run.model_format == "safetensors"

    def test_explicit_llama_cpp_backend_uses_gguf(self):
        run = self._run_pipeline(backend="llama_cpp")
        assert run.backend == "llama_cpp"
        assert run.model_format == "gguf"

    def test_native_backend_deploys_manifest_and_skips_gguf_conversion(self):
        self.pipeline._converter.convert = MagicMock(return_value="should-not-be-used")

        run = self._run_pipeline(backend="vllm", model_format="safetensors", task_type="coding")

        assert run.success
        assert run.backend == "vllm"
        assert run.model_format == "safetensors"
        assert run.output_model_path.endswith("native_adapter")
        assert run.model_manifest_path.endswith(".vetinari-training-manifest.json")
        self.pipeline._converter.convert.assert_not_called()
    ...
    def test_check_requirements_has_models_dir(self):
        result = self.pipeline.check_requirements()
        assert "models_dir" in result

    def test_check_requirements_has_native_models_dir(self):
        result = self.pipeline.check_requirements()
        assert "native_models_dir" in result

    def test_auto_base_model_uses_native_model_dir_not_gguf(self, tmp_path):
        import vetinari.training.pipeline as pipeline_mod

        gguf_dir = tmp_path / "gguf"
        native_dir = tmp_path / "native"
        gguf_dir.mkdir()
        native_model = native_dir / "qwen-native"
        native_model.mkdir(parents=True)
        (gguf_dir / "larger.gguf").write_bytes(b"0" * 2048)
        (native_model / "config.json").write_text('{"model_type": "qwen2"}', encoding="utf-8")
        (native_model / "model.safetensors").write_bytes(b"1")

        with (
            patch.object(pipeline_mod, "_MODELS_DIR", gguf_dir),
            patch.object(pipeline_mod, "_NATIVE_MODELS_DIR", native_dir),
        ):
            resolved = self.pipeline._resolve_base_model("auto")

        assert resolved == str(native_model)

    def test_model_revision_resolves_from_native_manifest(self, tmp_path):
        native_model = tmp_path / "native" / "vllm" / "safetensors" / "owner--repo" / ("f" * 40)
        native_model.mkdir(parents=True)
        (native_model / ".vetinari-download.json").write_text(
            json.dumps({"revision": "f" * 40}),
            encoding="utf-8",
        )

        assert self.pipeline._resolve_model_revision(str(native_model)) == "f" * 40

    def test_forgetting_check_receives_model_and_dataset_paths(self):
        self.pipeline._regularizer.should_stop_training = MagicMock(return_value=False)

        run = self._run_pipeline()

        assert run.success
        self.pipeline._regularizer.should_stop_training.assert_called_once()
        base_model, dataset_path = self.pipeline._regularizer.should_stop_training.call_args.args
        assert "Qwen2.5-Coder" in base_model
        assert str(dataset_path).endswith(".jsonl")

    def test_replay_buffer_add_receives_dataset_records_not_path_string(self):
        self.pipeline._replay_buffer.create_mixed_dataset = MagicMock(return_value=None)
        self.pipeline._replay_buffer.add = MagicMock()
        self.pipeline._replay_buffer.save = MagicMock()

        run = self._run_pipeline()

        assert run.success
        self.pipeline._replay_buffer.add.assert_called_once()
        replay_payload = self.pipeline._replay_buffer.add.call_args.args[0]
        assert isinstance(replay_payload, list)
        assert replay_payload
        assert isinstance(replay_payload[0], dict)
        assert not isinstance(replay_payload, str)
...
class TestDataCuratorEdgeCases:
...
class TestGGUFConverterEdgeCases:
...
class TestLocalTrainerEdgeCases:
    ...
    def test_lora_r_in_unsloth_script(self):
        ...
        with patch.object(self.trainer, "check_available", return_value=avail):
            with patch("subprocess.run", return_value=proc):
                self.trainer.train_qlora("model", self.dataset_path, self.tmpdir, lora_r=32, use_unsloth=True)
    ...
    def test_learning_rate_in_trl_script(self):
        ...
        with patch.object(self.trainer, "check_available", return_value=avail):
            with patch("subprocess.run", return_value=proc):
                self.trainer.train_qlora("model", self.dataset_path, self.tmpdir, learning_rate=1e-4, use_unsloth=False)
