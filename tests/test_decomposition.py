...
import vetinari.planning.decomposition as decomp_module
from vetinari.planning.decomposition import (
    DEFAULT_MAX_DEPTH,
    MAX_MAX_DEPTH,
    MIN_MAX_DEPTH,
    SEED_MIX,
...
class TestCriteriaConstants:
    def test_dod_has_three_levels(self):
        from vetinari.planning.decomposition import _DOD_CRITERIA

        assert set(_DOD_CRITERIA.keys()) == {"Light", "Standard", "Hard"}

    def test_dor_has_three_levels(self):
        from vetinari.planning.decomposition import _DOR_CRITERIA
...
class TestSubtaskSpec:
    def test_required_fields(self):
        ...
        assert spec.agent_type == AgentType.WORKER.value
        assert spec.depth == 1

    def test_default_lists_are_empty(self):
        spec = SubtaskSpec(
            subtask_id="s1", parent_task_id="root", description="x", agent_type=AgentType.WORKER.value, depth=0
        )
        assert spec.inputs == []
        assert spec.outputs == []
    ...
    def test_lists_are_independent_per_instance(self):
        s1 = SubtaskSpec("a", "root", "x", AgentType.WORKER.value, 0)
        s2 = SubtaskSpec("b", "root", "y", AgentType.WORKER.value, 0)
        s1.inputs.append("in1")
        assert s2.inputs == []

    def test_created_at_is_iso_string(self):
        spec = SubtaskSpec("s1", "root", "desc", AgentType.WORKER.value, 0)
...
class TestDecompositionEvent:
    ...
    def test_timestamp_is_iso_string(self):
        evt = DecompositionEvent(event_id="e1", plan_id="p", task_id="t", depth=0, seeds_used=[], subtasks_created=0)
        parsed = datetime.fromisoformat(evt.timestamp)
        assert isinstance(parsed, datetime)

    def test_seeds_used_stored(self):
        evt = DecompositionEvent(
            event_id="e1", plan_id="p", task_id="t", depth=1, seeds_used=["web_app", "api_service"], subtasks_created=5
        )
...
class TestDecompositionEngineInit:
    ...
    def test_all_templates_have_required_keys(self):
        ...
        for tmpl in engine._templates:
            assert required.issubset(set(tmpl.keys())), f"Template {tmpl.get('template_id')} missing keys"
...
class TestGetTemplates:
    ...
    def test_combined_keyword_and_agent_filter(self, engine):
        results = engine.get_templates(keywords=["data"], agent_type=AgentType.WORKER.value)
        assert all(t["agent_type"] == AgentType.WORKER.value for t in results)
        assert all(any(kw in t["keywords"] for kw in ["data"]) for t in results)
...
class TestGetCriteriaMethods:
    ...
    @pytest.mark.parametrize("tier", ["Standard", "Light", "Hard"])
    def test_get_dod_known_tiers_return_nonempty_list(self, engine, tier):
        criteria = engine.get_dod_criteria(tier)
    ...
    @pytest.mark.parametrize("tier", ["Standard", "Light", "Hard"])
    def test_get_dor_known_tiers_return_nonempty_list(self, engine, tier):
        criteria = engine.get_dor_criteria(tier)
        assert isinstance(criteria, list)
        assert len(criteria) >= 1
...
# ---------------------------------------------------------------------------
# DecompositionEngine.decompose_task — happy path (ForemanAgent mock)
# ---------------------------------------------------------------------------
...
class TestDecomposeTaskWithPlanner:
    ...
    def test_returns_list_on_success(self, engine):
        ...
        with (
            patch("vetinari.planning.decomposition.DecompositionEngine.decompose_task", wraps=engine.decompose_task),
            patch(
                "vetinari.agents.get_foreman_agent",
            ...
            subtasks = engine.decompose_task("Build a database service", plan_id="plan_1")
    ...
    def test_planner_subtask_fields_mapped(self, engine):
        ...
        mock_planner.execute.return_value = self._make_planner_result(task_data)
        with (
            patch("vetinari.agents.get_foreman_agent", return_value=mock_planner),
    ...
    def test_max_depth_clamped_to_min(self, engine):
        ...
        with patch.object(engine, "_keyword_decompose", side_effect=tracking_kw):
            ...
            with patch("vetinari.agents.get_foreman_agent", side_effect=Exception("no planner")):
    ...
    def test_fallback_to_keyword_on_planner_exception(self, engine):
        with patch("vetinari.agents.get_foreman_agent", side_effect=ImportError("no module")):
...
class TestGetDecompositionHistory:
    ...
    def test_history_accumulates(self, engine):
        with patch("vetinari.agents.get_foreman_agent", side_effect=ImportError("no planner")):
