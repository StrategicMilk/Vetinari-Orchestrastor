...
and uses the direct BuilderAgent fallback only when the file tool is absent.
"""
...
class _FakeFileOpsTool(Tool):
    ...
    def __init__(self):
        from vetinari.tool_interface import ToolParameter
        ...
        metadata = ToolMetadata(
            ...
            parameters=[
                ToolParameter(name="operation", type=str, description="Operation type", required=True),
                ToolParameter(name="path", type=str, description="File path", required=True),
                ToolParameter(name="content", type=str, description="File content", required=False),
            ],
        )
...
@pytest.fixture
def make_registry():
    """Return a factory that creates a ToolRegistry populated with given tools.

    Cannot use tests.factories because the tools registered are locally-defined
    classes specific to this test module.

    Returns:
        A callable that accepts *Tool instances and returns a ToolRegistry.
    """

    def _factory(*tools: Tool) -> ToolRegistry:
...
class TestBuilderToolWriteFile:
    def test_uses_file_operations_tool_when_available(self, make_registry):
        from vetinari.agents.builder_agent import BuilderAgent

        fake_tool = _FakeFileOpsTool()
        registry = make_registry(fake_tool)
    ...
    def test_blocks_direct_write_when_tool_fails(self, make_registry):
        from vetinari.agents.builder_agent import BuilderAgent
        ...
        registry = make_registry(_FailingFileOps())
        agent = BuilderAgent()
        agent.initialize({"tool_registry": registry})
...
class TestResearcherToolSearch:
    def test_uses_web_search_tool_when_available(self, make_registry):
    ...
    def test_falls_back_when_tool_returns_failure(self, make_registry):
        from vetinari.agents.consolidated.researcher_agent import ConsolidatedResearcherAgent
        ...
        registry = make_registry(_FailingSearch())
        agent = ConsolidatedResearcherAgent()
        agent.initialize({"tool_registry": registry})
    ...
    def test_result_format_matches_search_helper(self, make_registry):
