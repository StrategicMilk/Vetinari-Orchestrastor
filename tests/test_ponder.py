...
class TestPonderEngine:
    ...
    def test_context_score(self):
        ...
        # Low context needed — model meets minimum threshold
        reqs = {"context_needed": 8192}
    ...
    def test_policy_penalty_multiplicative_scores_positive(self):
        """B.5: Policy penalty must be multiplicative so all scores stay in (0, 1].

        With the old additive approach (total -= 1.0) a model with a weighted sum
        of 0.6 would get total = -0.4, which is invalid for Thompson sampling.
        The multiplicative fix (total * 0.3) produces 0.18 — still positive.
        """
        from vetinari.models.ponder import PonderEngine
        ...
        models = [
            {
                ...
                "context_length": 8192,
                "quantization": "q4_k_m",
                "tags": ["policy_sensitive"],
            },
        ]
        ranking = engine.score_models(models, "analyze legal contracts for compliance", top_n=1)

        assert len(ranking.rankings) == 1
        score = ranking.rankings[0].total_score
        # Score must be positive — negative would crash Thompson sampling
        assert score > 0.0, f"Policy-penalized score must be positive, got {score}"
        # Score must not exceed 1.0 (upper bound of normalized range)
...
class TestCloudProviders:
    ...
    def test_cloud_providers_config(self):
        """Cloud providers should be properly configured (loaded from YAML config)."""
        from vetinari.models.model_pool import _load_cloud_providers

        providers = _load_cloud_providers()

        assert "huggingface_inference" in providers
    ...
    def test_cloud_provider_health_with_tokens(self):
        ...
        with patch.dict(
            os.environ,
            {"HF_HUB_TOKEN": "test-hf-token", "CLAUDE_API_KEY": "test-claude-key", "GEMINI_API_KEY": "test-gemini-key"},
        ):
            health = ModelPool.get_cloud_provider_health()
...
class TestModelSearchCloud:
    ...
    def test_search_cloud_providers(self):
        ...
        with patch.dict(os.environ, {"CLAUDE_API_KEY": "test-key", "GEMINI_API_KEY": "test-key"}):
            engine = ModelSearchEngine()
...
class TestPonderConfig:
    ...
    def test_enable_ponder_model_search_default(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            from vetinari.models.ponder import ENABLE_PONDER_MODEL_DISCOVERY

            # Default should be True
            assert ENABLE_PONDER_MODEL_DISCOVERY is True
...
class TestPonderAPI:
    ...
    def test_ponder_health_endpoint(self, mock_get):
        ...
        data = response.json()
        assert data["providers"] == {"huggingface_inference": True, "replicate": False}
    ...
    def test_ponder_templates_endpoint(self, mock_get):
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["templates"] == {"coding": "...", "research": "..."}
...
class TestFallbackBehavior:
    ...
    def test_score_models_with_cloud_no_search(self):
        ...
        with patch("vetinari.models.ponder.ENABLE_PONDER_MODEL_DISCOVERY", False):
            models = [
    ...
    def test_cloud_fallback_graceful(self):
        ...
        from vetinari.models.ponder import _get_model_discovery_candidates
...
class TestPonderThompsonIntegration:
    """Tests for PonderEngine._get_thompson_score — data-driven scoring."""

    def test_thompson_score_returns_none_for_cold_start_arm(self):
        """Returns None when arm has fewer than 10 pulls (cold start)."""
        from unittest.mock import MagicMock, patch
        ...
        engine = PonderEngine()
        mock_arm = MagicMock()
        mock_arm.total_pulls = 5
        mock_arm.mean = 0.8

        mock_selector = MagicMock()
        mock_selector._lock = __import__("threading").Lock()
        mock_selector._arms = {"test-model:general": mock_arm}

        with patch("vetinari.learning.model_selector.get_thompson_selector", return_value=mock_selector):
            result = engine._get_thompson_score("test-model", "general")

        assert result is None

    def test_thompson_score_returns_mean_for_well_observed_arm(self):
        """Returns arm.mean when arm has >= 10 pulls."""
        from unittest.mock import MagicMock, patch
    ...
    def test_score_models_uses_thompson_score_as_memory_score(self):
        """score_models overrides mem_score with Thompson mean when well-observed."""
        from unittest.mock import patch

        from vetinari.models.ponder import PonderEngine

        engine = PonderEngine()
        model = {"id": "known-model", "tags": ["coding"], "context_len": 8192, "quantization": "q4_k_m"}

        with patch.object(engine, "_get_thompson_score", return_value=0.9) as mock_ts:
            ranking = engine.score_models([model], "write some code")

        mock_ts.assert_called_once_with("known-model", "coding")
        assert ranking.rankings[0].memory_score == 0.9
