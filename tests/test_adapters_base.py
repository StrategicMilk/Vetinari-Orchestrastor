...
from vetinari.adapters.base import (
    ProviderType, ProviderConfig, ModelInfo, InferenceRequest,
    InferenceResponse, ProviderAdapter,
)
...
class TestModelInfo(unittest.TestCase):
    def test_creation(self):
        ...
        self.assertEqual(m.cost_per_1k_tokens, 0.0)
        self.assertFalse(m.free_tier)

    def test_custom_fields(self):
        m = _make_model(latency_ms=500, free_tier=True, cost=0.002)
        self.assertEqual(m.latency_estimate_ms, 500)
        self.assertTrue(m.free_tier)
...
class TestProviderAdapterScoring(unittest.TestCase):
    ...
    def test_score_combined_penalty_lower_than_ideal(self):
        ...
        self.assertLessEqual(score_bad, score_good)
    ...
    def test_score_free_tier_bonus(self):
        ...
        self.assertGreaterEqual(s_free, s_paid)
        self.assertGreater(s_free - s_paid + 0.001, 0)  # at least non-negative
    ...
    def test_repr(self):
        self.assertIn("ConcreteAdapter", repr(self.adapter))
...
class TestAdapterReprAndInit(unittest.TestCase):
    def test_init_sets_attributes(self):
        ...
        self.assertEqual(a.endpoint, "http://ep")
