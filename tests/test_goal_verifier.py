...
import vetinari.validation.goal_verifier as gv_module
from tests.factories import make_evaluator_mock_result, make_security_mock_result
...
def _mock_quality_agent(mock_agent):
    ...
    with (
        patch("vetinari.agents.get_inspector_agent", return_value=mock_agent),
        patch("vetinari.agents.contracts.AgentTask", return_value=MagicMock()),
    ):
...
class TestGetCorrectiveTasks:
    ...
    def test_missing_feature_task_content(self):
        ...
        assert task["assigned_agent"] == AgentType.WORKER.value
    ...
    def test_security_task_has_critical_priority(self):
        ...
        assert sec_task["assigned_agent"] == AgentType.INSPECTOR.value
    ...
    def test_add_tests_task_fields(self):
        ...
        assert task["priority"] == "high"
        assert task["assigned_agent"] == AgentType.INSPECTOR.value
...
class TestGoalVerificationReportToDict:
    ...
    def test_features_serialized_as_list_of_dicts(self):
        report = GoalVerificationReport(project_id="p", goal="g")
        report.features = [FeatureVerification("Auth", True, 0.9, "Found in auth.py", "auth.py", "minor")]
        d = report.to_dict()
...
class TestVerifyFeatures:
    ...
    def test_feature_found_in_output(self, verifier):
        ...
        results = verifier._verify_features("goal", ["authentication system"], output, [])
    ...
    def test_feature_not_in_output(self, verifier):
        ...
        results = verifier._verify_features("goal", ["authentication token validation"], output, [])
        assert len(results) == 1
    ...
    def test_confidence_between_zero_and_one(self, verifier):
        results = verifier._verify_features("goal", ["some feature"], "some feature output", [])
        for r in results:
    ...
    def test_task_outputs_searched_too(self, verifier):
        ...
        results = verifier._verify_features("goal", ["authentication"], final_output, task_outputs)
        assert results[0].implemented is True

    def test_blank_feature_skipped(self, verifier):
        results = verifier._verify_features("goal", ["", "  ", "real feature"], "real feature is here", [])
        assert len(results) == 1
    ...
    def test_high_confidence_when_all_keywords_present(self, verifier):
        ...
        results = verifier._verify_features("goal", ["pagination sorting filtering search results"], output, [])
...
class TestCheckAvoidList:
    ...
    def test_item_present_in_output_is_violation(self, verifier):
        violations = verifier._check_avoid_list(["jQuery"], "We used jQuery for the frontend.")
        assert "jQuery" in violations

    def test_item_absent_not_violation(self, verifier):
        violations = verifier._check_avoid_list(["jQuery"], "We used vanilla JS only.")
        assert violations == []
    ...
    def test_multiple_violations(self, verifier):
        violations = verifier._check_avoid_list(
            ["deprecated_api", "legacy_code"], "uses deprecated_api and legacy_code throughout"
        )
    ...
    def test_only_present_items_returned(self, verifier):
        violations = verifier._check_avoid_list(["present", "absent"], "only present is here")
...
class TestLlmEvaluation:
    ...
    def test_returns_dict_on_success(self, verifier):
        ...
        mock_evaluator.execute.return_value = make_evaluator_mock_result()
        with _mock_quality_agent(mock_evaluator):
            result = verifier._llm_evaluation("Build an API", "output text", ["feature1"], [])
        assert isinstance(result, dict)

    def test_returns_none_on_import_error(self, verifier):
        with patch("vetinari.agents.get_inspector_agent", side_effect=ImportError("no module")):
    ...
    def test_returns_none_on_general_exception(self, verifier):
        with patch("vetinari.agents.get_inspector_agent", side_effect=RuntimeError("unexpected")):
            result = verifier._llm_evaluation("goal", "output", [], [])
        assert result is None
...
class TestSecurityCheck:
    ...
    def test_returns_three_tuple(self, verifier):
        ...
        mock_auditor.execute.return_value = make_security_mock_result(findings=[], score=100)
    ...
    def test_critical_finding_fails(self, verifier):
        mock_auditor = MagicMock()
        mock_auditor.execute.return_value = make_security_mock_result(
            findings=[{"severity": "critical", "id": "SQL_INJECTION"}],
            score=40,
        )
        with _mock_quality_agent(mock_auditor):
    ...
    def test_low_severity_does_not_fail(self, verifier):
        mock_auditor = MagicMock()
        mock_auditor.execute.return_value = make_security_mock_result(
            findings=[{"severity": "low", "id": "INFO_LEAK"}],
            score=90,
        )
        with _mock_quality_agent(mock_auditor):
            passed, _findings, _score = verifier._security_check("code", [])
    ...
    def test_import_error_fails_closed(self, verifier):
        with patch("vetinari.agents.get_inspector_agent", side_effect=ImportError("no module")):
    ...
    def test_score_normalized_from_100(self, verifier):
        mock_auditor = MagicMock()
        mock_auditor.execute.return_value = make_security_mock_result(findings=[], score=75)
        with _mock_quality_agent(mock_auditor):
    ...
    def test_medium_severity_does_not_fail(self, verifier):
        mock_auditor = MagicMock()
        mock_auditor.execute.return_value = make_security_mock_result(
            findings=[{"severity": "medium", "id": "CSRF"}],
            score=80,
        )
        with _mock_quality_agent(mock_auditor):
            passed, _findings, _score = verifier._security_check("code", [])
...
class TestGoalVerifierVerify:
    ...
    def _run_verify(self, verifier, **kwargs):
        defaults = {
            ...
            "final_output": "def get_users(): return []\nimport pytest\ndef test_users():\n    users = get_users()\n    assert isinstance(users, list)",
            "required_features": [],
            "things_to_avoid": [],
            "task_outputs": [],
            "expected_outputs": [],
    ...
    def test_tests_present_detected(self, verifier):
        report = self._run_verify(verifier, final_output="import pytest\ndef test_foo(): pass")
    ...
    def test_not_compliant_when_security_fails(self, verifier):
        with (
            ...
            patch.object(verifier, "_security_check", return_value=(False, [{"severity": "critical"}], 0.0)),
        ):
            report = verifier.verify(project_id="p", goal="g", final_output="code with issues")
        assert report.fully_compliant is False
    ...
    def test_null_task_outputs_handled_gracefully(self, verifier):
        with (
            ...
            report = verifier.verify(project_id="p", goal="g", final_output="code", task_outputs=None)
        assert isinstance(report, GoalVerificationReport)
...
class TestComplianceScoreFormula:
    ...
    def test_security_failure_deducts_from_score(self):
        ...
        with (
            ...
            patch.object(verifier, "_security_check", return_value=(False, [{"severity": "critical"}], 0.5)),
        ):
            report = verifier.verify(
                project_id="p",
                goal="g",
    ...
    def test_fully_compliant_requires_security_pass(self):
        ...
        with (
            ...
            patch.object(verifier, "_security_check", return_value=(False, [{"severity": "high"}], 0.0)),
        ):
            report = verifier.verify(project_id="p", goal="g", final_output="code")
        assert report.fully_compliant is False
    ...
    def test_no_tests_reduces_score_vs_tests_present(self):
        ...
        with (
            ...
            r_no_tests = verifier.verify(project_id="p", goal="g", final_output="pure code no tests")
        with (
            patch.object(
                verifier,
                "_llm_evaluation",
            ...
            r_with_tests = verifier.verify(project_id="p", goal="g", final_output="import pytest\ndef test_x(): pass")
