...
class TestEvaluateDocument:
    ...
    def test_hedging_lowers_accuracy(self):
        ...
        accuracy = next(
            ds for ds in report.dimension_scores if ds.dimension == "accuracy"
        )
    ...
    def test_placeholders_lower_correctness(self):
        ...
        report = evaluate_document(text)
        correctness = next(
            ds for ds in report.dimension_scores if ds.dimension == "correctness"
        )
    ...
    def test_filler_phrases_lower_conciseness(self):
        ...
        report = evaluate_document(text)
        conciseness = next(
            ds for ds in report.dimension_scores if ds.dimension == "conciseness"
        )
    ...
    def test_code_blocks_boost_technical_depth(self):
        ...
        depth_with = next(
            ds for ds in report_with.dimension_scores if ds.dimension == "technical_depth"
        )
        depth_without = next(
            ds for ds in report_without.dimension_scores if ds.dimension == "technical_depth"
        )
    ...
    def test_mixed_list_markers_lower_consistency(self):
        ...
        report = evaluate_document(text)
        consistency = next(
            ds for ds in report.dimension_scores if ds.dimension == "consistency"
        )
    ...
    def test_long_document_without_headings_lowers_organization(self):
        ...
        report = evaluate_document(text)
        org = next(
            ds for ds in report.dimension_scores if ds.dimension == "organization"
        )
    ...
    def test_custom_profile_override(self):
        from vetinari.validation.document_types import DocumentProfile
