...
class TestQualityScore:
    ...
    def test_quality_score_defaults(self):
        ...
        assert qs.correctness == 0.0
        assert qs.method == "unmeasured"
...
class TestQualityScorer:
    ...
    @pytest.fixture
    def scorer(self):
        """Create a QualityScorer backed by the per-test isolated database."""
    ...
    def test_heuristic_score_coding(self, scorer):
        ...
        assert score.correctness == 0.55  # structural coding check
        assert score.style == 0.65  # has docstring
        assert "test_coverage" in score.dimensions
        assert score.dimensions["test_coverage"] == 0.7  # has assert
    ...
    def test_heuristic_score_research(self, scorer):
        ...
        assert 0.0 <= score.overall_score <= 1.0
        assert score.dimensions.get("source_quality", 0) == 0.75  # has http
        assert score.dimensions.get("actionability", 0) == 0.75  # has sections + actionable
    ...
    def test_heuristic_score_empty_output(self, scorer):
        ...
        # Empty string matches the fallback-rejection guard, which fires before the heuristic path
        assert any("empty" in issue.lower() or "fallback" in issue.lower() for issue in score.issues)
    ...
    def test_heuristic_score_short_output(self, scorer):
        ...
        assert score.dimensions.get("completeness", 1.0) == 0.15
        assert any("Very short output" in issue for issue in score.issues)
    ...
    def test_heuristic_score_coding_no_def(self, scorer):
        ...
        assert score.correctness == 0.3  # no def/class
    ...
    def test_heuristic_score_research_no_sources(self, scorer):
        ...
        # Input mentions "references" so no missing-citation issue; score is low due to no URLs
    ...
    def test_score_persistence(self, scorer, tmp_path):
        ...
        # Re-create scorer sharing the same isolated DB (via conftest fixture)
        from vetinari.learning.quality_scorer import QualityScorer
        ...
        assert history[0].method in ("heuristic", "unmeasured")
    ...
    def test_get_model_average(self, scorer):
        ...
        assert 0.0 <= avg <= 1.0
        assert avg > 0.0  # Both outputs have "def", so scored > 0; avg must be positive
        assert isinstance(avg, float)

    def test_get_model_average_no_data(self, scorer):
        avg = scorer.get_model_average("nonexistent_model")
        assert avg == 0.0  # No data returns 0.0 (unmeasured)
...
class TestFeedbackLoop:
    ...
    def test_record_outcome_no_crash(self, feedback_loop):
        ...
        result = feedback_loop.record_outcome(
            task_id="t1",
            model_id="m1",
            task_type="coding",
            quality_score=0.85,
        ...
        assert result is None  # record_outcome returns None
    ...
    def test_record_outcome_with_mock_memory(self, feedback_loop):
        ...
        # EMA with alpha=0.2: (1 - 0.2) * 0.7 + 0.2 * (0.5 * 1.0 + 0.5 * 0.9) = 0.56 + 0.19 = 0.75
        assert abs(update_dict["success_rate"] - 0.75) < 0.01
    ...
    def test_ema_alpha_value(self, feedback_loop):
        ...
        assert FeedbackLoop.EMA_ALPHA == 0.2  # Optimized: less reactive to single outliers
...
class TestBetaArm:
    ...
    def test_initial_mean(self):
        ...
        arm = ThompsonBetaArm(model_id="m1", task_type="coding")
    ...
    def test_sample_in_range(self):
        ...
        arm = ThompsonBetaArm(model_id="m1", task_type="coding", alpha=5.0, beta=3.0)
        random.seed(42)
...
class TestThompsonSamplingSelector:
    ...
    def test_state_persistence(self, selector, tmp_path):
        ...
        # Force state save (normally happens at atexit or every 100 observations)
        selector._save_state()

        # Create new selector pointing at same state dir.
        # Re-assert the env var to guard against interference from other tests.
        from vetinari.learning.model_selector import ThompsonSamplingSelector
    ...
    def test_thread_safety(self, selector):
        ...
        threads = [threading.Thread(target=update_arm, args=(f"m{i}", 50)) for i in range(5)]
        for t in threads:
            t.start()
        for t in threads:
            t.join()
