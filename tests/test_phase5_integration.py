...
for _stubname in (
    ...
    "vetinari.analytics.sla",
    "vetinari.analytics.forecasting",
...
@pytest.fixture
def adapter():
    """Create a ConcreteAdapter with a minimal local ProviderConfig."""
    cfg = ProviderConfig(
...
class TestAdapterCostTelemetry:
    ...
    def test_cost_entry_recorded(self, adapter):
    ...
    def test_cost_entry_with_metadata(self, adapter):
    ...
    def test_cost_entry_with_task_id(self, adapter):
        req = InferenceRequest(
            model_id="test-model",
            prompt="hello",
...
class TestAdapterSLATelemetry:
    ...
    def test_sla_records_latency(self, adapter):
    ...
    def test_sla_records_success(self, adapter):
        tracker = get_sla_tracker()
        tracker.register_slo(
            SLOTarget(
                name="test-success",
        ...
        from vetinari.analytics.sla import SLAReport

        report = tracker.get_report("test-success")
        assert isinstance(report, SLAReport)
        assert report.slo.name == "test-success"


class TestAdapterForecasterTelemetry:
    ...
    def test_forecaster_receives_latency(self, adapter):
        req = InferenceRequest(model_id="test-model", prompt="hello")
        adapter.infer(req)
    ...
    def test_forecaster_receives_tokens(self, adapter):
        req = InferenceRequest(model_id="test-model", prompt="hello")
        adapter.infer(req)
...
class TestAdapterAnomalyTelemetry:
    ...
    def test_anomaly_detector_runs(self, adapter):
        ...
        # History may or may not contain anomalies, but the detector must be live
        assert isinstance(detector, AnomalyDetector)


class TestOrchestratorDefaultSLOs:
...
class TestOrchestratorAutoTuner:
    ...
    def test_auto_tuner_applies_max_concurrent(self):
        try:
            ...
            pytest.skip("orchestrator or auto_tuner not available")
...
class TestTelemetryNeverCrashesInference:
    ...
    def test_telemetry_failure_suppressed(self, adapter):
        req = InferenceRequest(model_id="m", prompt="x")
