...
class TestAdapterManagerRegistration:
    ...
    def test_register_provider(self):
        ...
        with patch.object(self.manager.registry, 'create_adapter', return_value=self.mock_adapter):
            adapter = self.manager.register_provider(config, "lmstudio-1")

            assert adapter == self.mock_adapter
            assert "lmstudio-1" in self.manager._metrics
            assert "lmstudio-1" in self.manager._provider_fallback_order
