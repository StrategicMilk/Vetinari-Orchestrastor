...
def _make_tracker_report(entries):
    ...
    Mocks a CostReport-like object with by_model mapping from entries.
    """
    tracker = MagicMock()
    report = MagicMock()
    report.total_requests = len(entries)
    report.by_model = {e["model_id"]: e.get("cost_usd", 0.0) for e in entries}
    tracker.get_report.return_value = report
    return tracker
...
class TestGetEfficiencies:
    def test_returns_one_entry_per_model(self):
        opt = _make_optimizer()
        with (
            patch("vetinari.learning.cost_optimizer.CostOptimizer._get_cost_tracker", return_value=None),
    ...
    def test_uses_cost_from_tracker(self):
        ...
        with (
            patch.object(opt, "_get_cost_tracker", return_value=tracker),
            patch("vetinari.learning.model_selector.get_thompson_selector") as ts_mock,
        ):
    ...
    def test_cost_tracker_exception_falls_back_to_zero_cost(self):
        ...
        with (
            patch.object(opt, "_get_cost_tracker", return_value=bad_tracker),
            patch("vetinari.learning.model_selector.get_thompson_selector") as ts_mock,
        ):
...
class TestSelectCheapestAdequate:
    ...
    def test_max_cost_filter_excludes_expensive_models(self):
        ...
        opt = self._opt_with_efficiencies(effs)
        result = opt.select_cheapest_adequate("test", ["pricey", "affordable"], max_cost_usd=0.10)
    ...
    def test_fallback_to_highest_quality_when_max_cost_filters_all(self):
        ...
        result = opt.select_cheapest_adequate("test", ["model-a", "model-b"], max_cost_usd=0.10)
