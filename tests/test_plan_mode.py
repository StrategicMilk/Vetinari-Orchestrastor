...
def _make_code_agent_task(description, **kwargs):
    """Stub for make_code_agent_task — returns a MagicMock AgentTask."""
    task = MagicMock()
    task.task_id = "task_stub"
...
_coding_agent_stub.make_code_agent_task = _make_code_agent_task
...
import vetinari.planning.plan_mode as plan_mode_module
from vetinari.exceptions import PlanningError
from vetinari.planning.plan_mode import (
    DEPTH_CAP,
    DRY_RUN_ENABLED,
...
def _make_engine(memory=None):
    ...
    with patch("vetinari.planning.plan_mode.get_memory_store", return_value=memory):
...
def _make_plan_candidate(domain: TaskDomain = TaskDomain.CODING) -> PlanCandidate:
    """Return a minimal PlanCandidate for subtask-creation tests."""
...
class TestPlanModeEngineInit:
    ...
    def test_init_calls_get_memory_store_when_none_given(self):
        memory = _make_mock_memory()
        with patch("vetinari.planning.plan_mode.get_memory_store", return_value=memory) as mock_get:
...
class TestGenerateCandidates:
    ...
    def test_respects_max_candidates_of_one(self, engine):
        candidates = engine._generate_candidates("goal", "", TaskDomain.GENERAL, max_candidates=1, depth_cap=16)
        assert len(candidates) == 1

    def test_respects_max_candidates_of_two(self, engine):
        candidates = engine._generate_candidates("goal", "", TaskDomain.GENERAL, max_candidates=2, depth_cap=16)
        assert len(candidates) == 2

    def test_caps_at_three_even_if_max_is_higher(self, engine):
        # The implementation uses min(max_candidates, 3)
        candidates = engine._generate_candidates("goal", "", TaskDomain.GENERAL, max_candidates=10, depth_cap=16)
        assert len(candidates) <= 3

    def test_candidates_have_increasing_risk_scores(self, engine):
        candidates = engine._generate_candidates("goal", "", TaskDomain.CODING, max_candidates=3, depth_cap=16)
        scores = [c.risk_score for c in candidates]
        assert scores == sorted(scores)

    def test_low_risk_level_assigned_for_low_score(self, engine):
        candidates = engine._generate_candidates("goal", "", TaskDomain.CODING, max_candidates=1, depth_cap=16)
    ...
    def test_candidates_have_domain_set(self, engine):
        candidates = engine._generate_candidates("goal", "", TaskDomain.DATA_PROCESSING, max_candidates=2, depth_cap=16)
        for c in candidates:
            assert TaskDomain.DATA_PROCESSING in c.domains

    def test_depth_cap_respected_in_candidates(self, engine):
        candidates = engine._generate_candidates("goal", "", TaskDomain.CODING, max_candidates=3, depth_cap=5)
        for c in candidates:
...
class TestCreateSubtasksFromCandidate:
    ...
    def test_returns_list_of_subtasks(self, engine):
        candidate = _make_plan_candidate(TaskDomain.CODING)
    ...
    def test_subtasks_have_correct_plan_id(self, engine):
        candidate = _make_plan_candidate()
    ...
    def test_subtasks_have_pending_status(self, engine):
        ...
        subtasks = engine._create_subtasks_from_candidate(candidate, "plan_x")
        for s in subtasks:
            assert s.status == StatusEnum.PENDING
    ...
    def test_subtasks_have_domain_set(self, engine):
        candidate = _make_plan_candidate(TaskDomain.DOCS)
        subtasks = engine._create_subtasks_from_candidate(candidate, "plan_x")
        for s in subtasks:
...
class TestGeneratePlan:
    ...
    def test_dry_run_low_risk_plan_auto_approved(self, engine_and_memory):
        ...
        assert isinstance(plan.approved_at, str)
        assert "T" in plan.approved_at
    ...
    def test_plan_has_chosen_plan_id(self, engine_and_memory):
        ...
        assert isinstance(plan.chosen_plan_id, str)

    def test_plan_persisted_to_memory(self, engine_and_memory):
        engine, memory = engine_and_memory
        self._gen(engine)
        assert memory.write_plan_history.call_count >= 1
        call_args = memory.write_plan_history.call_args[0]
        assert len(call_args) >= 1  # plan data was passed
    ...
    def test_workflow_learner_consulted(self, engine_and_memory):
        ...
        goal = "design a migration"
        with patch(
            "vetinari.learning.workflow_learner.get_workflow_learner",
            return_value=mock_learner,
        ):
            plan = self._gen(engine, goal=goal)
        mock_learner.get_recommendations.assert_called_once_with(goal)
    ...
    # explain_agent tests removed — explain_agent was deleted in Phase 10 remediation
    # Explanation generation is disabled; will be wired through Operations.creative_writing mode
...
class TestUpdateSubtaskStatus:
    def test_returns_true_on_success(self):
        ...
        eng, _ = _make_engine(memory)
        result = eng.update_subtask_status(plan.plan_id, subtask.subtask_id, StatusEnum.COMPLETED)
    ...
    def test_returns_false_when_subtask_not_found(self):
        ...
        memory.query_subtasks.return_value = []
        eng, _ = _make_engine(memory)
        result = eng.update_subtask_status("plan", "bad_id", StatusEnum.COMPLETED)
    ...
    def test_status_value_written_to_memory(self):
        ...
        memory.write_subtask_memory.return_value = True
        eng, _ = _make_engine(memory)
        eng.update_subtask_status(plan.plan_id, subtask.subtask_id, StatusEnum.RUNNING)
        written = memory.write_subtask_memory.call_args[0][0]
        assert written["status"] == StatusEnum.RUNNING.value
    ...
    def test_outcome_written_when_provided(self):
        ...
        memory.write_subtask_memory.return_value = True
        eng, _ = _make_engine(memory)
        eng.update_subtask_status(plan.plan_id, subtask.subtask_id, StatusEnum.COMPLETED, outcome="OK")
        written = memory.write_subtask_memory.call_args[0][0]
    ...
    def test_updated_at_timestamp_set(self):
        ...
        memory.write_subtask_memory.return_value = True
        eng, _ = _make_engine(memory)
        eng.update_subtask_status(plan.plan_id, subtask.subtask_id, StatusEnum.COMPLETED)
        written = memory.write_subtask_memory.call_args[0][0]
...
class TestLogApprovalDecision:
    ...
    def test_returns_true_when_store_available(self, engine):
        ...
        with patch("vetinari.memory.unified.get_unified_memory_store", return_value=mock_store):
    ...
    def test_calls_store_remember(self, engine):
        captured = []
        mock_store = MagicMock()
        mock_store.remember.side_effect = captured.append
        with patch("vetinari.memory.unified.get_unified_memory_store", return_value=mock_store):
            engine.log_approval_decision("plan_1", "sub_1", True, "alice")
        assert len(captured) == 1
        entry = captured[0]
        content = json.loads(entry.content)
        assert entry.summary == "Approved task sub_1 by alice"
        assert entry.provenance == "plan_approval_api"
        assert content["approval_status"] == "approved"
        assert content["task_id"] == "sub_1"
        assert content["approver"] == "alice"

    def test_returns_false_when_store_is_none(self, engine):
        with patch("vetinari.memory.unified.get_unified_memory_store", return_value=None):
            result = engine.log_approval_decision("plan_1", "sub_1", True, "alice")
        assert result is False

    def test_returns_false_on_exception(self, engine):
        with patch("vetinari.memory.unified.get_unified_memory_store", side_effect=RuntimeError("boom")):
            result = engine.log_approval_decision("plan_1", "sub_1", True, "alice")
        assert result is False
...
class TestAutoApproveIfLowRisk:
    ...
    def test_auto_approve_logs_decision(self, engine):
        ...
        content = json.loads(entry.content)
        assert content["approval_status"] == "approved"
        assert content["approver"] == "system_auto"
        assert content["risk_score"] == 0.1
        assert "low risk" in content["reason"]
...
class TestExecuteCodingTask:
    ...
    def test_returns_success_true_when_agent_available(self, engine):
        ...
        with patch("vetinari.coding_agent.get_coding_agent", return_value=mock_agent):
            with patch("vetinari.memory.unified.get_unified_memory_store", return_value=MagicMock()):
...
class TestPersistPlan:
    def test_writes_plan_to_memory(self):
        ...
        written_plan = memory.write_plan_history.call_args[0][0]
        assert written_plan["plan_id"] == plan.plan_id
        assert written_plan["goal"] == plan.goal
        assert json.loads(written_plan["plan_json"])["plan_id"] == plan.plan_id

    def test_writes_subtask_for_each_subtask(self):
        memory = _make_mock_memory()
        eng, _ = _make_engine(memory)
        plan = _simple_plan()
        plan.subtasks.append(Subtask(subtask_id="subtask_001", plan_id=plan.plan_id, domain=TaskDomain.GENERAL))
        eng._persist_plan(plan)
...
class TestEdgeCases:
    ...
    def test_risk_level_critical_threshold(self):
        ...
        subtasks = [Subtask(subtask_id=f"s_{i}", cost_estimate=30.0, depth=10) for i in range(40)]
        plan.subtasks = subtasks
