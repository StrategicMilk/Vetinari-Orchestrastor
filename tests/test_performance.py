...
class TestModelRouterPerformance:
    ...
    def test_complexity_estimation_latency(self):
        """estimate_complexity() should be < 1ms p99."""
        from vetinari.routing.model_router import estimate_complexity

        desc = "Implement a security audit for the authentication module with OWASP compliance"
        stats = _measure_latency(lambda: estimate_complexity(desc), iterations=1000)
        assert stats["p99"] < 1.0, f"estimate_complexity p99={stats['p99']:.3f}ms exceeds 1ms"

    def test_complexity_router_latency(self):
        """classify_complexity() should be < 2ms p99."""
        from vetinari.routing.complexity_router import classify_complexity
...
class TestSchemaPerformance:
    ...
    def test_validate_output_latency(self):
        ...
        from vetinari.schemas.agent_outputs import validate_output
...
class TestSecurityScanPerformance:
    ...
    def test_heuristic_scan_latency(self):
        """Scanning 500 lines of code should be < 50ms p99."""
        from vetinari.agents.consolidated.quality_agent import QualityAgent

        # Build a mock agent (skip LLM init)
        with patch.object(QualityAgent, '__init__', lambda self, *a, **kw: None):
