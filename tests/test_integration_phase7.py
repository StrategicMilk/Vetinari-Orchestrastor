...
class TestTelemetryToDashboard:
    ...
    def test_adapter_metrics_visible_in_snapshot(self):
        ...
        for i in range(5):
            tel.record_adapter_latency("openai", "gpt-4", 100.0 + i * 20, success=True, tokens_used=100)
    ...
    def test_plan_metrics_in_snapshot(self):
        ...
        tel = get_telemetry_collector()
        for _ in range(5):
            tel.record_plan_decision("approve", risk_score=0.2, approval_time_ms=80.0)
        tel.record_plan_decision("reject", risk_score=0.8, approval_time_ms=200.0)
...
class TestCostToAnalytics:
    ...
    def test_end_to_end_cost_report(self):
        ...
        tracker.set_pricing("openai", "gpt-4", ModelPricing(input_per_1k=0.03, output_per_1k=0.06))
...
class TestSLABreachDetection:
    ...
    def test_success_rate_compliance(self):
        ...
        assert report.current_value == pytest.approx(100.0)
        assert report.is_compliant


class TestForecastingCapacityPipeline:
...
class TestLogAggregatorPipeline:
...
class TestAdapterRegistryPipeline:
    ...
    def test_create_local_adapter_and_score_model(self):
        ...
        cfg = ProviderConfig(provider_type=ProviderType.LOCAL, name="lms", endpoint="local")
        ...
        score = adapter.score_model_for_task(
            m,
            {
        ...
        best_adapter, best_model = AdapterRegistry.find_best_model({"required_capabilities": ["code_gen"]})
