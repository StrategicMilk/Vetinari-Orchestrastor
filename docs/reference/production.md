...
All runtime configuration is read from environment variables or `.env`.
...
|---|---|---|
| `LMSTUDIO_HOST` | LM Studio API base URL | `http://127.0.0.1:1234` |
| `VETINARI_SECRET_KEY` | Flask secret key (random 32+ chars) | `openssl rand -hex 32` |
...
```bash
python examples/dashboard_rest_api_example.py --debug
```
...
gunicorn "vetinari.dashboard.rest_api:create_app()" \
    --bind 0.0.0.0:5000 \
    --workers 4 \
    --timeout 60 \
...
ExecStart=/opt/vetinari/venv/bin/gunicorn \
    "vetinari.dashboard.rest_api:create_app()" \
    --bind 0.0.0.0:5000 \
    --workers 4
Restart=always
RestartSec=5
...
2. **CORS**: restrict `Access-Control-Allow-Origin` in production by
   editing `rest_api.py`'s `add_cors_headers` function.

3. **HTTPS**: always terminate TLS at Nginx/load balancer before the app.

4. **Secret key**: set `VETINARI_SECRET_KEY` to a cryptographically random
   value; never commit it to version control.

5. **Database**: `vetinari_memory.db` (SQLite) contains agent memory.
   Restrict file permissions: `chmod 600 vetinari_memory.db`.
...
→ { "status": "healthy", "timestamp": "...", "service": "vetinari-dashboard" }
```

Use this with load balancers, Kubernetes liveness/readiness probes, or uptime
monitors.
...
|---|---|---|
| Gunicorn workers | 4 | `(2 × CPU_cores) + 1` |
...
```bash
# Back up SQLite memory database
cp vetinari_memory.db backups/vetinari_memory_$(date +%Y%m%d).db
...
# Restore
cp backups/vetinari_memory_20260303.db vetinari_memory.db
```
...
python vetinari/migrations/upgrade_subtask_schema.py
...
| `gunicorn` workers restarting | Timeout on slow model calls | Increase `--timeout` to 120 s |
| SQLite locked | Multiple processes accessing DB | Switch to PostgreSQL for multi-process deployments |
