...
class ApprovalEntry:
    ...
    entry_type: MemoryEntryType = MemoryEntryType.APPROVAL
...
```http
GET /api/plan/{plan_id}/subtasks/{subtask_id}/approval
```

### List Pending Approvals

```http
GET /api/plan/approvals/pending
```
...
3. **Access Control**: Approval endpoints require admin token
4. **Rate Limiting**: Implement rate limiting to prevent approval spam
