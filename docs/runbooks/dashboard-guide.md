...
| REST API | `vetinari.dashboard.rest_api` | Flask HTTP wrapper around the API |
...
```bash
# 1. Install dependencies (Flask is the only addition to Phase 3 deps)
pip install flask

# 2. Launch the dashboard server
python -m vetinari.dashboard.rest_api
...
```bash
python vetinari/dashboard/rest_api.py              # default: 0.0.0.0:5000
python vetinari/dashboard/rest_api.py 127.0.0.1 8080 --debug
```

### As part of an existing Flask app
...
The `create_app()` factory registers all API routes and the `/dashboard` UI
route on the returned Flask application. It points `template_folder` and
`static_folder` at the `ui/` directory automatically.
...
The Elasticsearch, Splunk, and Datadog backends require `requests`:
