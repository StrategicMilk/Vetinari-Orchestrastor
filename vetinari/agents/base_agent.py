...
class BaseAgent(ABC):
    ...
    def _get_base_prompt_framework(cls) -> str:
        ...
        return (
            ...
            "## Vetinari Project Standards\n\n"
            "These standards are mandatory for all Vetinari agents regardless of "
            "runtime environment or model provider.\n\n"
            "DO NOT CHEAT: Always do things correctly, even when it is harder or "
            "slower. No shortcuts, no workarounds, no placeholder implementations, "
            "no superficial patches, no skipping steps that feel tedious. If a task "
            "touches ten files, touch all ten. If a fix requires updating every "
            "caller, update every caller. Fix root causes — never delete or weaken "
            "a test to make code pass. Correctness is not negotiable.\n\n"
            "IMPORTS: Enums from vetinari.types, specs from vetinari.agents.contracts, "
            "interfaces from vetinari.agents.interfaces. Never redefine locally.\n"
            "LOGGING: Use logging.getLogger(__name__) with %-style formatting. "
            "Never print() in production code.\n"
            "ERROR HANDLING: Specific exceptions only, chain with 'from'. "
            "Never bare except.\n"
            ...
            "COMPLETENESS: No TODO, pass bodies, NotImplementedError, placeholder "
            "strings, or commented-out code in production.\n"
            "ANNOTATIONS: from __future__ import annotations at the top of every "
            "vetinari/ file.\n"
