...
Configuration routes (credentials, rules, model-relay, ponder) live in
config_routes.py.
"""
...
@system_mgmt_bp.route('/api/decisions', methods=['POST'])
...
@system_mgmt_bp.route('/api/sandbox/status', methods=['GET'])
...
@system_mgmt_bp.route('/api/code-search', methods=['GET'])
def api_code_search():
...
@system_mgmt_bp.route('/api/search/index', methods=['POST'])
...
@system_mgmt_bp.route('/api/search/status', methods=['GET'])
...
@system_mgmt_bp.route('/api/adr', methods=['GET'])
def api_adr_list():
    try:
...
@system_mgmt_bp.route('/api/adr/<adr_id>', methods=['GET'])
...
@system_mgmt_bp.route('/api/adr', methods=['POST'])
def api_adr_create():
    try:
        from vetinari.adr import adr_system
        data, err = validate_json_request()
...
@system_mgmt_bp.route('/api/adr/<adr_id>', methods=['PUT'])
...
@system_mgmt_bp.route('/api/adr/<adr_id>/deprecate', methods=['POST'])
...
@system_mgmt_bp.route('/api/adr/propose', methods=['POST'])
def api_adr_propose():
    try:
        from vetinari.adr import adr_system
        data, err = validate_json_request()
...
@system_mgmt_bp.route('/api/adr/propose/accept', methods=['POST'])
def api_adr_propose_accept():
    try:
        ...
        adr = adr_system.accept_proposal(proposal, data.get('title', 'Proposed Decision'), data.get('category', 'architecture'))
        return jsonify(adr.to_dict())
    except Exception as e:
        return jsonify({"error": str(e)}), 500


@system_mgmt_bp.route('/api/adr/statistics', methods=['GET'])
...
@system_mgmt_bp.route('/api/adr/is-high-stakes', methods=['GET'])
...
@system_mgmt_bp.route('/api/generate-image', methods=['POST'])
...
@system_mgmt_bp.route('/api/sd-status', methods=['GET'])
def api_sd_status():
...
@system_mgmt_bp.route('/api/training/stats', methods=['GET'])
...
@system_mgmt_bp.route('/api/training/export', methods=['POST'])
def api_training_export():
    ...
    try:
        ...
        if err:
            return err
        export_format = data.get('format', 'sft')
        collector = get_training_collector()
