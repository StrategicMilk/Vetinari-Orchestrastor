...
class CodeSandbox:
    ...
    def _wrap_python_code(self, code: str, input_data: dict[str, Any] | None = None) -> str:
...
class CodeExecutor:
    ...
    def run_and_validate(
        self, code: str, expected_output: str | None = None, error_pattern: str | None = None
    ) -> dict[str, Any]:
        ...
        # Overall validation
        validation["all_validations_passed"] = all(v for v in validation["validations"].values())
...
if __name__ == "__main__":
    ...
    result = sandbox.execute_python("print('Hello from sandbox!')")  # noqa: VET035
    ...
    result = sandbox.execute_python("print(f'Got: {{INPUT_DATA}}')")  # noqa: VET035
    logger.info("Output: %s", result.output)
    ...
    logger.info("Success: %s", result.success)
    logger.info("Error: %s...", result.error[:100])

    # Test code validation
    logger.info("=== Code validation ===")
    executor = CodeExecutor(sandbox)
    validation = executor.run_and_validate("print('Test passed')", expected_output="Test passed")  # noqa: VET035
