...
class CocoIndexAdapter(CodeSearchAdapter):
    ...
    def _check_availability(self) -> bool:
        if not shutil.which("uvx"):
            return False
        try:
            result = subprocess.run(["uvx", "--version"], capture_output=True, timeout=5)  # noqa: S607
    ...
    def search(self, query: str, limit: int = 10, filters: dict | None = None) -> list[CodeSearchResult]:
        ...
        try:
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=60, cwd=self.root_path)  # noqa: S603
    ...
    def index_project(self, project_path: str, force: bool = False) -> bool:
        ...
        try:
            result = subprocess.run(cmd, capture_output=True, timeout=300, cwd=project_path)  # noqa: S603
            return result.returncode == 0
        except Exception as e:
