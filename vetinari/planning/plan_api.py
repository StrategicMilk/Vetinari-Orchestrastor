...
from .plan_mode import PLAN_MODE_DEFAULT, PLAN_MODE_ENABLE, get_plan_engine
from .plan_types import PlanApprovalRequest, PlanGenerationRequest, TaskDomain
...
def approve_plan(plan_id):
    ...
    try:
        ...
        try:
            from vetinari.memory import DUAL_MEMORY_AVAILABLE, MemoryEntry, MemoryEntryType, get_dual_memory_store
...
def get_plan_subtasks(plan_id):
    ...
    try:
        ...
        return jsonify(
            {"success": True, "plan_id": plan_id, "subtasks": [s.to_dict() for s in subtasks], "count": len(subtasks)}
        )
...
def get_plan_explanations(plan_id):
    ...
    try:
        from vetinari.agents.explain_agent import PlanExplanation, get_explain_agent
        ...
        if sanitized and explanation_data:
            ...
            return jsonify({"success": True, "plan_id": plan_id, "explanation": sanitized_exp.to_dict()})

        return jsonify(
            {"success": True, "plan_id": plan_id, "explanation": explanation_data if explanation_data else None}
        )
    ...
    try:
        ...
        for st_data in subtasks_data:
            ...
            try:
                artifact = agent.run_task(task)
                results.append(
                    {"subtask_id": st_data.get("subtask_id"), "success": True, "artifact": artifact.to_dict()}
                )
            except Exception as task_err:
                results.append({"subtask_id": st_data.get("subtask_id"), "success": False, "error": str(task_err)})
