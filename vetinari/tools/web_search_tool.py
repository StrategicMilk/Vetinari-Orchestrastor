...
class WebSearchTool:
    ...
    def search(
        self, query: str, max_results: int = 5, language: str = "en", time_range: str | None = None
    ) -> SearchResponse:
        ...
        response = SearchResponse(
            ...
            provenance=[{"backend": self.backend_name, "timestamp": datetime.now().isoformat(), "query": query}],
        )
    ...
    def _search_serpapi(self, query: str, max_results: int, language: str, time_range: str) -> list[SearchResult]:
        ...
        try:
            ...
            params = {"q": query, "api_key": self.serapi_key, "num": max_results, "engine": "google"}
            if time_range:
    ...
    def _search_tavily(self, query: str, max_results: int, language: str, time_range: str) -> list[SearchResult]:
        ...
        try:
            ...
            data = {"query": query, "max_results": max_results, "include_answer": True, "include_raw_content": False}

            resp = requests.post("https://api.tavily.com/search", json=data, headers=headers, timeout=30)
            resp.raise_for_status()
            data = resp.json()
    ...
    def _search_searxng(self, query: str, max_results: int, language: str, time_range: str) -> list[SearchResult]:
        ...
        try:
            ...
            params = {"q": query, "format": "json", "engines": "google,duckduckgo,bing", "language": language}
    ...
    def research_topic(self, topic: str, aspects: list[str] | None = None) -> dict[str, Any]:
        ...
        """
        if aspects is None:
            aspects = [f"{topic} overview", f"{topic} implementation", f"{topic} best practices", f"{topic} tutorials"]
    ...
    def verify_claim(
        ...
        supporting_results = self.multi_source_search(claim, max_results=max_search_results).results
        negation_query = f"NOT ({claim}) OR problems with {claim}"
        contradicting_results = self.search(negation_query, max_results=3).results
        ...
        for r in supporting_results:
            ...
            entry = {"title": r.title, "url": r.url, "overlap": round(overlap, 2), "reliability": r.source_reliability}
            if overlap > 0.2:
