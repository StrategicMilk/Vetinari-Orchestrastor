...
def _load_agent_capabilities(dotpath: str) -> Optional[List[str]]:
...
class CapabilityAuditor:
    ...
    _instance:   Optional["CapabilityAuditor"] = None
    ...
    def register_documented(self, agent_name: str,
    ...
    def audit_all(self, only_documented: bool = True) -> List[CapabilityFinding]:
        ...
        """
        with self._lock:
            names = list(self._documented.keys()) if only_documented \
                else list(_AGENT_MODULES.keys())
    ...
    def get_drift_findings(self) -> List[CapabilityFinding]:
        ...
        return [f for f in self.audit_all(only_documented=False)
                if f.is_drift]
