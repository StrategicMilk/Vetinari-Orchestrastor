...
def create_app(debug: bool = False) -> Flask:
    ...
    def health_check() -> tuple[dict[str, Any], int]:
        ...
        return jsonify(
            {"status": "healthy", "timestamp": datetime.now(timezone.utc).isoformat(), "service": "vetinari-dashboard"}
        ), 200
    ...
    def get_timeseries() -> tuple[dict[str, Any], int]:
        ...
        try:
            ...
            if metric not in valid_metrics:
                return jsonify({"error": f"Invalid metric '{metric}'. Valid metrics: {valid_metrics}"}), 400
    ...
    def search_traces() -> tuple[dict[str, Any], int]:
        ...
        try:
            ...
            return jsonify({"count": len(traces), "traces": [t.to_dict() for t in traces]}), 200
        except Exception as e:
