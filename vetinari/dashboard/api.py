...
class DashboardAPI:
    ...
    def get_latest_metrics(self) -> MetricsSnapshot:
        ...
        with self._lock:
            ...
            adapter_summary = {
                "total_providers": len(set(m.provider for m in adapter_metrics.values())),
    ...
    def get_timeseries_data(self, metric: str, timerange: str = "24h", 
                           provider: Optional[str] = None) -> Optional[TimeSeriesData]:
        """
    ...
    def _get_adapter_latency_timeseries(self, timerange: str, 
                                       provider: Optional[str] = None) -> TimeSeriesData:
    ...
    def _get_success_rate_timeseries(self, timerange: str, 
                                     provider: Optional[str] = None) -> TimeSeriesData:
    ...
    def _get_token_usage_timeseries(self, timerange: str, 
                                    provider: Optional[str] = None) -> TimeSeriesData:
    ...
    def search_traces(self, trace_id: Optional[str] = None, 
        ...
        with self._lock:
            if trace_id:
                ...
                results = sorted(self._trace_list, 
                               key=lambda t: t.start_time, 
                               reverse=True)[:limit]
            
            return results
    
    def get_trace_detail(self, trace_id: str) -> Optional[TraceDetail]:
        """
        ...
        with self._lock:
            try:
                ...
                trace_info = TraceInfo(
                    ...
                    root_operation=trace_detail.spans[0].get("operation", "unknown") if trace_detail.spans else "unknown"
                )
