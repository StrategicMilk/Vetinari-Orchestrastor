...
from flask import Flask, jsonify, render_template_string, request, send_from_directory  # noqa: E402
...
app = Flask(
    __name__, template_folder=str(PROJECT_ROOT / "ui" / "templates"), static_folder=str(PROJECT_ROOT / "ui" / "static")
)
...
from vetinari.web.admin_routes import bp as admin_bp  # noqa: E402
from vetinari.web.adr_routes import bp as adr_bp  # noqa: E402
from vetinari.web.decomposition_routes import bp as decomposition_bp  # noqa: E402
from vetinari.web.ponder_routes import bp as ponder_bp  # noqa: E402
from vetinari.web.rules_routes import bp as rules_bp  # noqa: E402
from vetinari.web.training_routes import bp as training_bp  # noqa: E402
...
def api_create_plan():
    ...
    try:
        ...
        current_config.get("default_models", [])
        current_config.get("fallback_models", [])
...
def api_new_project():
    ...
    try:
        ...
        current_config.get("default_models", [])
        current_config.get("fallback_models", [])
        current_config.get("uncensored_fallback_models", [])
        current_config.get("memory_budget_gb", 32)
            ...
            try:
                ...
                if _plan_agent is not None:
                    ...
                    if _plan_result.success and _plan_result.output:
                        model_response = (
                            _plan_result.output if isinstance(_plan_result.output, str) else str(_plan_result.output)
                        )
            except Exception as _orch_err:
                logger.warning(
                    "Orchestrator pipeline unavailable for planning (%s), falling back to adapter", _orch_err
                )
...
Implement this task. Output the code as code blocks with filenames."""
                        ...
                        try:
                            ...
                            from vetinari.orchestration.two_layer import TwoLayerOrchestrator
                            from vetinari.types import AgentType as _AgentType
                            ...
                            logger.warning(
                                "Orchestrator agent unavailable for task %s (%s), falling back to adapter", task_id, _te
                            )
...
def api_read_file(project_id):
    try:
        ...
        logger.info("IO Read: %s (project: %s)", target_path, project_id)
...
def api_write_file(project_id):
    ...
    try:
        ...
        # Log the IO operation
        logger.info("IO Write: %s (project: %s, size: %d)", target_path, project_id, len(content))
...
def api_plans_list():
    try:
        ...
        return jsonify({"plans": [p.to_dict() for p in plans], "total": len(plans), "limit": limit, "offset": offset})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def api_plan_start(plan_id):
    ...
    try:
        ...
        return jsonify({"plan_id": plan.plan_id, "status": plan.status, "started_at": plan.updated_at})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def api_plan_pause(plan_id):
    ...
    try:
        ...
        return jsonify({"plan_id": plan.plan_id, "status": plan.status, "paused_at": plan.updated_at})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def api_plan_resume(plan_id):
    ...
    try:
        ...
        return jsonify({"plan_id": plan.plan_id, "status": plan.status, "resumed_at": plan.updated_at})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def api_plan_cancel(plan_id):
    ...
    try:
        ...
        return jsonify({"plan_id": plan.plan_id, "status": plan.status, "cancelled_at": plan.updated_at})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def api_get_subtasks(plan_id):
    try:
        ...
        return jsonify({"plan_id": plan_id, "subtasks": [s.to_dict() for s in subtasks], "total": len(subtasks)})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def api_get_assignments(plan_id):
    try:
        ...
        return jsonify({"plan_id": plan_id, "assignments": assignments, "total": len(assignments)})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def api_override_assignment(plan_id, subtask_id):
    try:
        ...
        subtask = subtask_tree.update_subtask(
            plan_id, subtask_id, {"assigned_agent": assigned_agent, "status": "assigned"}
        )
...
def api_templates():
    try:
        ...
        return jsonify(
            {"templates": templates, "total": len(templates), "version": version or template_loader.default_version()}
        )
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
try:
    ...
    logger.warning("Plan Mode API not available: %s", e)
except Exception as e:
    logger.warning("Failed to register Plan Mode API: %s", e)
