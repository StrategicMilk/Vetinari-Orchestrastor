...
class ToolRegistry:
    ...
    def list_tools_for_mode(self, mode: ExecutionMode) -> list[Tool]:
        ...
        return [tool for tool in self._tools.values() if mode in tool.metadata.allowed_modes]
    ...
    def get_tools_requiring_permission(self, permission: ToolPermission) -> list[Tool]:
        ...
        return [tool for tool in self._tools.values() if permission in tool.metadata.required_permissions]
