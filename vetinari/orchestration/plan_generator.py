...
class PlanGenerator:
    ...
    def _decompose_goal(self, goal: str, max_depth: int) -> List[Dict]:
        ...
        try:
            ...
            logger.warning(
                f"PlannerAgent decomposition failed: {e}, using keyword fallback"
            )
    ...
    def _keyword_decomposition(self, goal: str) -> List[Dict]:
        ...
        is_code = any(
            k in goal_lower
            for k in [
                "code", "implement", "build", "create", "program", "app",
                "web", "software",
            ]
        )
        is_research = any(
            k in goal_lower
            for k in ["research", "analyze", "investigate", "study", "review"]
        )
        is_docs = any(
            k in goal_lower
            for k in ["document", "readme", "explain", "write", "report"]
        )
