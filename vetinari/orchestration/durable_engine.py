...
from vetinari.orchestration.types import Checkpoint, ExecutionEvent, TaskNode
...
class DurableExecutionEngine:
    ...
    def __init__(self, checkpoint_dir: str | None = None, max_concurrent: int = 4, default_timeout: float = 300.0):
    ...
    def execute_plan(self, graph: ExecutionGraph, task_handler: Callable | None = None) -> dict[str, Any]:
        ...
        results = {"plan_id": plan_id, "total_tasks": len(graph.nodes), "completed": 0, "failed": 0, "task_results": {}}
    ...
    def _execute_task(self, graph: ExecutionGraph, task: TaskNode) -> dict[str, Any]:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                # Emit event
                self._emit_event("task_completed", task_id, {"status": "completed", "attempts": attempt + 1})
                ...
                try:
                    ...
                    task_type_str = (
                        task.task_type.lower() if hasattr(task, "task_type") and task.task_type else "general"
                    )
        ...
        # Emit event
        self._emit_event("task_failed", task_id, {"status": "failed", "error": last_error, "attempts": max_attempts})
