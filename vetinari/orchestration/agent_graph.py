...
class AgentGraph:
    ...
    def __init__(
        ...
        self._quality_reviewed_agents: set = (
            quality_reviewed_agents if quality_reviewed_agents is not None else {AgentType.BUILDER}
        )
        ...
        for attempt in range(node.max_retries + 1):
            try:
                ...
                if not verification.passed and attempt < node.max_retries:
                    ...
                    issues_text = "; ".join(
                        i.get("message", str(i)) if isinstance(i, dict) else str(i) for i in (verification.issues or [])
                    )
                    logger.warning(
                        f"[AgentGraph] {task.id} verification failed: {issues_text} — injecting feedback and retrying"
                    )
    ...
    def inject_task(
        ...
        new_node.dependents = {nid for nid, n in plan.nodes.items() if new_task.id in n.dependencies}
    ...
    def _apply_maker_checker(
        ...
        for iteration in range(self._MAKER_CHECKER_MAX_ITERATIONS):
            ...
            try:
                ...
                logger.warning(
                    f"[AgentGraph] Maker-checker: QUALITY rejected {task.id} (iteration {iteration + 1}): {issues_text}"
                )
    ...
    def _find_delegate(self, task: Task, exclude: AgentType = None) -> AgentType | None:
    ...
    def get_execution_plan(self, plan_id: str) -> ExecutionPlan | None:
    ...
    def get_agent_by_capability(self, capability: str) -> Any | None:
        ...
        try:
            ...
            _AGENT_TYPE_PRIORITY = {
                "ORCHESTRATOR": 10,
                "CONSOLIDATED_RESEARCHER": 10,
                "CONSOLIDATED_ORACLE": 10,
                "ARCHITECT": 10,
                "QUALITY": 10,
                "OPERATIONS": 10,
                "PLANNER": 8,
