"""Legacy MemoryStore — plan history, subtask, and model-performance persistence.

Use ``DualMemoryStore`` for new code.  This module is kept for callers that
rely on the ``MemoryStore`` / ``get_memory_store`` API.
"""
...
PLAN_MEMORY_DB_PATH = os.environ.get("PLAN_MEMORY_DB_PATH", str(_DEFAULT_DATA_DIR / "vetinari_memory.db"))
PLAN_RETENTION_DAYS = int(os.environ.get("PLAN_RETENTION_DAYS", "90"))
PLAN_ADMIN_TOKEN = os.environ.get("PLAN_ADMIN_TOKEN", "")


class MemoryStore:
    """Legacy memory store for plan history and model performance.
    ...
    def _init_sqlite(self):
        try:
            ...
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_subtask_plan_id ON SubtaskMemory(plan_id)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_plan_created_at ON PlanHistory(created_at)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_subtask_domain ON SubtaskMemory(domain)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_plan_status ON PlanHistory(status)")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_subtask_status ON SubtaskMemory(status)")
...
def get_memory_store() -> MemoryStore:
    ...
    if _memory_store is None:
        import os as _os
        use_json = _os.environ.get("PLAN_USE_JSON_FALLBACK", "false").lower() in ("1", "true", "yes")
