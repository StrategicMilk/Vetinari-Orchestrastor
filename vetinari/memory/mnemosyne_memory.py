...
class MnemosyneMemoryStore(IMemoryStore):
    ...
    def search(self, query: str, agent: Optional[str] = None,
        ...
        for memory in self._data.get("memories", []):
            ...
            if query_lower not in memory.get("content", "").lower() and \
               query_lower not in memory.get("summary", "").lower():
                continue
    ...
    def compact(self, max_age_days: Optional[int] = None) -> int:
        ...
        memories = [
            m for m in memories
            if not m.get("forgotten", False) and 
            (not max_age_days or m.get("timestamp", 0) >= cutoff)
        ]
