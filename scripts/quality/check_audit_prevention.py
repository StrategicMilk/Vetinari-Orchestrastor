...
def check_audit_prevention_workflow_wiring(root: Path) -> list[Finding]:
    ...
    text = path.read_text(encoding="utf-8")
    if "scripts/quality/check_audit_prevention.py" not in text:
        return [Finding(display_path, "audit-prevention workflow must run scripts/quality/check_audit_prevention.py")]
    return []
