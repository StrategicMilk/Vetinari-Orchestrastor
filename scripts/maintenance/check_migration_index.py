...
Usage:
    python scripts/maintenance/check_migration_index.py
"""
...
def check_phase7_artifacts() -> bool:
    ...
    artifacts = {
        ...
        "scripts/maintenance/check_doc_contract_alignment.py": "Alignment checker script",
        "scripts/maintenance/check_migration_index.py": "Migration index checker",
        "scripts/inspect/check_agent_capabilities.py": "Capability checker script",
        "scripts/quality/check_coverage_gate.py": "Coverage gate script",
...
def check_agent_implementations() -> bool:
    """Validate the 3-agent factory pipeline required by ADR-0061.

    Checks that Foreman, Worker, and Inspector agent files exist at their
    canonical paths, plus the shared support files they depend on.

    Returns:
        True when all 3 factory agent files are present.
    """
    _section("3-AGENT FACTORY PIPELINE (ADR-0061)")
    # Foreman = planner_agent.py (ForemanAgent class),
    # Worker  = consolidated/worker_agent.py (WorkerAgent class),
    # Inspector = consolidated/quality_agent.py (InspectorAgent class)
    agent_files = [
    ...
    # Key support files shared across the pipeline
    support_files = [
        "vetinari/agents/base_agent.py",
        "vetinari/agents/contracts.py",
        "vetinari/agents/interfaces.py",
    ]
    found_agents = sum(1 for p in agent_files if _check_file_exists(p))
    found_support = sum(1 for p in support_files if _check_file_exists(p))
    for p in agent_files:
        print(f"  {'OK' if _check_file_exists(p) else 'MISS'}  {p}")
    print()
    for p in support_files:
        print(f"  {'OK' if _check_file_exists(p) else 'MISS'}  {p}  (support)")
    print(f"\n  {found_agents}/{len(agent_files)} factory agents, {found_support}/{len(support_files)} support files")
    return found_agents == len(agent_files)
...
def main() -> int:
    ...
    checks = [
        ...
        ("3-Agent Factory Pipeline", check_agent_implementations),
