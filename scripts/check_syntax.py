...
Walks from the project root (one level above scripts/) so that vetinari/,
tests/, scripts/, and other top-level packages are all covered.
"""
...
def main() -> int:
    ...
    """
    # Go up one level from scripts/ to reach the project root so that
    # vetinari/, tests/, and all other packages are included in the walk.
